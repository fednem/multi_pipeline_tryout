...
performance <- table(out_class$classification, out_class$ground)
accuracy <- sum(diag(performance))/sum(performance)
sensitivity <- performance[2,2]/sum(performance[2,])
specificity <- performance[1,1]/sum(performance[1,])

out_weights <- out %>%
  map(~`$`(.,"weights"))

gm_by_fold <- out_weights %>%
  map(~`$`(.,"features_name")) %>%
  map(~str_detect(.,"gm")) %>%
  map_int(~sum(.))

gm_by_fold_ration <- out_weights %>%
  map(~`$`(.,"features_name")) %>%
  map(~str_detect(.,"gm")) %>%
  map_dbl(~mean(.))


wm_by_fold <- out_weights %>%
  map(~`$`(.,"features_name")) %>%
  map(~str_detect(.,"wm")) %>%
  map_int(~sum(.))

wm_by_fold_ration <- out_weights %>%
  map(~`$`(.,"features_name")) %>%
  map(~str_detect(.,"wm")) %>%
  map_dbl(~mean(.))


rs_by_fold <- out_weights %>%
  map(~`$`(.,"features_name")) %>%
  map(~str_detect(.,"rs")) %>%
  map_int(~sum(.))


rs_by_fold_ration <- out_weights %>%
  map(~`$`(.,"features_name")) %>%
  map(~str_detect(.,"rs")) %>%
  map_dbl(~mean(.))

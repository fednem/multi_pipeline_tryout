...
gm_info <- reshape_images_for_pipeline("E:/multi_pipeline_tryout-improvement_on_relieff/gm_combined/", "gm_mask.nii.gz", "s8")
gm_matrix <- gm_info$n_by_v_matrix
gm_img_dim <- gm_info$dim_img
gm_img_struct <- gm_info$img_struct
rm(gm_info)

wm_info <- reshape_images_for_pipeline("E:/multi_pipeline_tryout-improvement_on_relieff/wm_combined/", "wm_mask.nii.gz", "s8")
wm_matrix <- wm_info$n_by_v_matrix
wm_img_dim <- wm_info$dim_img
wm_img_struct <- wm_info$img_struct
rm(wm_info)

rs_info <- reshape_images_for_pipeline("E:/multi_pipeline_tryout-improvement_on_relieff/rs_dmn_mpfc_combined/",
...
out <- foreach(fold_index = 6:10, .inorder = FALSE, 

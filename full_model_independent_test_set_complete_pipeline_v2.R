...
wm_img_struct <- wm_info$img_struct
rm(wm_info)
...
rs_img_struct <- rs_info$img_struct
rm(rs_info)
...
wm_info_test <- reshape_images_for_pipeline("E:/multi_pipeline_tryout-improvement_on_relieff/wm_aix", "wm_mask.nii.gz", "s8")
wm_matrix_test <- wm_info_test$n_by_v_matrix
rm(wm_info_test)

rs_info_test <- reshape_images_for_pipeline("E:/multi_pipeline_tryout-improvement_on_relieff/rs_dmn_mpfc_aix",
                                       "gm_mask.nii.gz", "resampled")
...
wm_train <- wm_matrix
rs_train <- rs_matrix
gm_test <- gm_matrix_test
wm_test <- wm_matrix_test
rs_test <- rs_matrix_test
                             
outcome_train <- as.factor(outcome$outcome)
#create outcome for test set [KNOWN]
outcome_test <- as.factor(c(rep("HC",21), rep("NF1", 17)))
...
gm_var_thr$outcome <- outcome_train
wm_var_thr$outcome <- outcome_train
rs_var_thr$outcome <- outcome_train
...
rm(wm_bb, wm_object.pp3, wm_object.pp3_labelled)
...
gm_df_clusters <- gm_relieff %>%
  ...
  ungroup(.) %>%
  select(-subject)
  gm_relieff_features <- colnames(gm_relieff)[-1]
rm(gm_relieff)
...
wm_df_clusters <- wm_relieff %>%
  ...
  ungroup(.) %>%
  select(-subject)
wm_relieff_features <- colnames(wm_relieff)[-1]
rm(wm_relieff)
...
rs_df_clusters <- rs_relieff %>%
  ...
  ungroup(.) %>%
  select(-subject)
rs_relieff_features <- colnames(rs_relieff)[-1]
rm(rs_relieff)
